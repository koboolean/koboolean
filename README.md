...
Spring 기반 백엔드 개발을 중심으로 단순한 기능 구현을 넘어 **사용자가 실제로 필요로 하는 것이 무엇인지 고민하고, 이를 서비스로 풀어내는 과정에 집중하고 있습니다.**
문제를 해결할 때도 단순 구현이 아니라 **왜 필요한지, 어떤 방식이 더 나은 경험을 만드는지**를 함께 고민하며 구조를 설계합니다.
아이디어를 실제 서비스로 구현하고, **기획 → 설계 → 개발 → 배포까지 전 과정을 연결하는 것을 즐기는 개발자입니다.**
